...
class YysSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        for htmlItem in response.css('.game-list>li'):
            ...
            yield SplashRequest(item['href'], args={ "wait": 0.5 }, callback=self.parse_game_detail)
    ...
    def parse_game_detail(self, response):
        ...
        item['platforms'] = response.css('.game-platform-tag-list>li>a::text').getall()
        ...
        item['features'] = response.css('.game-info-rank-tags-list>li>a::text').getall()
        ...
        item['systems'] = list(set(response.css('[data-render="system"]>a::text').getall()))
        # item['sub_platforms'] = response.css('[data-render="sub_platform"]>a::text').get()
        item['minimal_config'] = response.css('[data-render="min_configuration"]::text').getall()
        item['recommend_config'] = response.css('[data-render="max_configuration"]::text').getall()
